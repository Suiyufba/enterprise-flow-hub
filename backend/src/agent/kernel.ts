...
export function buildSystemPrompt(input: AgentKernelInput): string {
  ...
  return [
    ...
    "- 飞书功能路由：群/私聊消息用 im_v1_chat_* 与 im_v1_message_*；文档/知识库用 docx_built_in_search、docx_v1_document_raw_content、wiki_v1_node_search；云空间用 drive_v1_file_list；多维表格用 bitable_v1_appTable_*；人员/部门用 contact_v3_user_* 与 contact_v3_department_*；任务用 task_v2_*；日程用 calendar_v4_*；审批用 approval_v4_*。先读后写，按 API 的真实结果回答。",
