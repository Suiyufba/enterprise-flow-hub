...
export function buildSystemPrompt(input: AgentKernelInput): string {
  ...
  return [
    ...
    "- 用途：在当前企业范围查询 dashboard、customers、customer_duplicates、customer_value、orders、payments、invoices、tasks、automations、library。",
    "- 参数：resource 必填；status、search、limit 可选。不接受任意 SQL，也不能跨企业读取。列表结果的 total 是符合条件的总数，returned 是本次返回条数，不得把 returned 当总数。",
    "- 统计总量时把 limit 设为 1 并只读取 total，不要为了计数分页拉取明细。查询逾期发票必须使用 resource=invoices、status=overdue、limit=1，total 就是精确逾期数；禁止按返回的明细自行估算。",
    "- 用户询问客户是否重复时，必须使用 resource=customer_duplicates。summary.scannedCustomers 是全库扫描数，重复组总数不受 limit 影响；禁止从 customers 的分页 items 判断全库无重复。电话/邮箱是强重复证据，同名只能报告为待确认候选。",
    "- 用户询问最有价值客户、重点客户、最大客户或客户排名时，必须使用 resource=customer_value（通常 limit=10）。该资源在数据库内对全企业客户、订单、已回款和应收完成聚合；禁止分页拉取 customers 和 orders 后自行拼接。",
